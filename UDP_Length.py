...
parser = argparse.ArgumentParser(
    description="Simple UDP Packet receiver that outputs packet length to stdout.",
...
parser.add_argument(
    ...
    "--When",
    action="store_true",
    help="Display the time that the packet was received",
)
parser.add_argument(
    "-E",
    "--Epoch",
    type=float,
    default=0.0,
    help="Display the total size of data that was received in that amount of time. In seconds. Note that generally this should be around 1/2 the output rate",
)
        ...
        if Verbose:
            sys.stderr.write(
                "Packet: {} From Address: {}:{} at {}\n".format(
                    Packets_In, addr[0], addr[1], rcv_time
                )
            )
            sys.stderr.flush()
